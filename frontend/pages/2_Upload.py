...
if st.button(" Process Invoice", type="primary", use_container_width=True):
    ...
    if check_invoice_hash(content_hash):
        st.warning("This invoice has already been processed (duplicate detected). Processing cancelled.")
        st.stop()
    ...
    # ── Cas 1 : échec de traitement (OCR/validation après 3 tentatives) ──────
    if not invoice:
        ...
        save_invoice({
            ...
            "validation_errors": ["Processing failed after maximum retries — manual review required"],
            "validation_warnings": [],
    ...
    # ── Cas 2 : rejet DiT (pas une facture) ──────────────────────────────────
    if not invoice.dit_is_invoice:
        st.error(
            f"This document is not an invoice — rejected by DiT "
            f"(score={invoice.dit_confidence:.3f})."
        )
        save_invoice({
            "vendor_name": "", "invoice_date": "", "invoice_id": "",
            "total_amount": None, "subtotal": None, "tax_amount": None,
    ...
    # ── Cas 3 : facture acceptée par DiT ─────────────────────────────────────
